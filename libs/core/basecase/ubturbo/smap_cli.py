...
class SmapCli(SmapNodeExecutor):
    ...
    def run(self, cmd: str, timeout=600, work_dir: str = "") -> str:
        result = self._ssh_host.run({"command": [f"echo {cmd} | nc -U /tmp/smap_nc_socket.sock"]})
        if result['stderr']:
    ...
    def smap_init(self, page_type: int) -> int:
        output = self.run(f"smap smap_init {page_type}")
        return self._get_func_ret_code(output)

    def smap_set_runmode(self, run_mode: int) -> int:
        output = self.run(f"smap set_smap_run_mode {run_mode}")
        return self._get_func_ret_code(output)

    def set_smap_remote_numa_info(self, src_nid: Optional[int], dst_nid: Optional[int],
                                  available_mem: Optional[int]) -> int:
        if src_nid is None:
            output = self.run(f"smap set_smap_remote_numa_info")
        else:
            output = self.run(f"smap set_smap_remote_numa_info {src_nid} {dst_nid} {available_mem}")
        return self._get_func_ret_code(output)

    def smap_stop(self) -> int:
        output = self.run(f"smap smap_stop")
        return self._get_func_ret_code(output)

    def smap_mig_out(self, msg: Optional[MigrateOutMsg], pid_type) -> int:
        if msg:
            output = self.run(f"smap smap_mig_out {str(msg)} {pid_type}")
        else:
            output = self.run(f"smap smap_mig_out {pid_type}")
        return self._get_func_ret_code(output)

    def smap_mig_out_memsize(self, msg: Optional[MigrateOutSizeMsg], pid_type) -> int:
        if msg:
            output = self.run(f"smap smap_mig_out_memsize {str(msg)} {pid_type}")
        else:
            output = self.run(f"smap smap_mig_out_memsize {pid_type}")
        return self._get_func_ret_code(output)

    def smap_mig_out_memsize_sync(self, msg: Optional[MigrateOutSizeMsg], pid_type, wait_time) -> int:
        if msg:
            output = self.run(f"smap smap_mig_out_sync {str(msg)} {pid_type} {wait_time}")
        else:
            output = self.run(f"smap smap_mig_out_sync {pid_type}")
        return self._get_func_ret_code(output)

    def smap_migrate_back(self, msg: Optional[MigrateBackMsg]):
        if msg:
            output = self.run(f"smap smap_mig_back {str(msg)}", timeout=600)
        else:
            output = self.run(f"smap smap_mig_back")
        return self._get_func_ret_code(output)

    def smap_remove(self, msg: Optional[RemoveMsg], pid_type: int) -> int:
        if msg:
            output = self.run(f"smap smap_remove {str(msg)} {pid_type}")
        else:
            output = self.run(f"smap smap_remove {pid_type}")
        return self._get_func_ret_code(output)

    def smap_black(self, proc_name: str) -> int:
        output = self.run(f"smap smap_black {proc_name}")
        return self._get_func_ret_code(output)

    def smap_unblack(self, proc_name: str) -> int:
        output = self.run(f"smap smap_unblack {proc_name}")
        return self._get_func_ret_code(output)

    def smap_enable_node(self, msg: Optional[EnableNodeMsg]) -> int:
        if msg:
            output = self.run(f"smap smap_enable {str(msg)}")
        else:
            output = self.run(f"smap smap_enable")
        return self._get_func_ret_code(output)
    ...
    def smap_query_vms_mem(self, flag: int) -> QueryMsg:
        output = self.run(f"smap smap_query_vms_mem {flag}")
    ...
    def smap_query_vm_freq(self, pid: int, data_flag: int, length: int) -> int:
        output = self.run(f"smap smap_query_vm_freq {pid} {data_flag} {length}")
        return self._get_func_ret_code(output)
    ...
    def smap_query_vm_cold_pages(self, pid: int, data_flag: int, length: int, no_print: int, page_start: int,
        ...
        :return: 返回冷热页数量
        """
        output = self.run(f"smap smap_query_vm_freq {pid} {data_flag} {length} {no_print} {page_start} {page_end}")
        rc = self._get_func_ret_code(output)
        if rc != 0:
            return rc
        match = re.search(r"nr_freq0: (\d+), hits: (\d+)", output)
    ...
    def smap_enable_process_migrate(self, pid_list: List[int], length: int, enable: int, flags: int):
        ...
        output = self.run(f"smap smap_enable_process_migrate {enable} {flags} {length} {pid_str}")
        return self._get_func_ret_code(output)

    def smap_migrate_pid_remote_numa(self, msg: Optional[MigratePidNumaMsg], migMode: int):
        if msg:
            output = self.run(f"smap smap_migrate_pid_remote_numa {str(msg)} {migMode}")
        else:
            output = self.run(f"smap smap_migrate_pid_remote_numa")
        return self._get_func_ret_code(output)

    def smap_migrate_remote_numa(self, msg: Optional[MigrateNumaMsg]) -> int:
        if msg:
            output = self.run(f"smap smap_migrate_remote_numa {str(msg)}")
        else:
            output = self.run(f"smap smap_migrate_remote_numa")
        return self._get_func_ret_code(output)
    ...
    def smap_add_process_tracking(self, pid_list: List[int], pid_time: List[int], length: int, scanType: int):
        ...
        output = self.run(f"smap smap_add_process_tracking {pid_str} {pid_time} {length} {scanType}")
        return self._get_func_ret_code(output)
    ...
    def smap_remove_process_tracking(self, pid_list: List[int], length: int, flag: int):
        pid_str = " ".join([str(pid) for pid in pid_list])
        output = self.run(f"smap smap_remove_process_tracking {pid_str} {length} {flag}")
        return self._get_func_ret_code(output)

    def smap_query_process_config(self, nid: int, flag: int, in_len: int, out_len: int) -> ProcessConfig:
        output = self.run(f"smap smap_query_process_config {nid} {flag} {in_len} {out_len}")
        rc = self._get_func_ret_code(output)
    ...
    def smap_set_smap_run_mode(self, mode: int):
        output = self.run(f"smap set_smap_run_mode {mode}")
        return "Set run mode success!" in output

    def read_proc_tracking_info(self, pid) -> List[SmapTrackInfo]:
        time.sleep(10)
        output = self.run(f"smap smap_query_freq_info {pid}")
        if output is None:
            return []
        track_info_list = []
        array_size = 0
    ...
    def smap_enable_adapt_mem(self, mode: int) -> bool:
        output = self.run(f"smap smap_enable_adapt_mem {mode}")
    ...
    def smap_query_numa_freq(self, length: int, numa_list: List[int]):
        ...
        output = self.run(f"smap smap_query_numa_freq {length} {numa_str}")
        rc = self._get_func_ret_code(output)
        if rc != 0:
            return rc
