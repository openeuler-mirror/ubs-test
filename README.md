...
需要拉�?ubs_comm 开源仓代码，编�?perf_test 测试工具�?
...
编译完成后会�?`build` 目录下得�?`hcom_perf` 二进制文件。HCOM 用例依赖两节点环境，需要将 `hcom_perf` 同时放在两个节点各自�?`/home/ubs-comm/hcom/perf_test` 文件夹下�?
...
1. 需确保执行节点已安装部�?ubs-openstack 插件
...
UBS Test 是一个专有的测试项目，用于对 UB ServiceCore 进行全面的多组件特性测试和集成测试。项目基�?pytest 框架构建，提供完整的测试基础设施和自动化测试能力�?

### 测试覆盖范围

| 测试模块     | 用例数量    | 测试内容                                     |
| -------- | ------- | ---------------------------------------- |
| hcom     | 30      | HCOM API 可用性测�?                          |
| ubse     | 16      | UB Pooling Management、UB Controller 管理测试 |
| ubsocket | 22      | Socket 通信功能测试（BRPC TCP/UB/MIX 协议�?       |
| ubturbo  | 41      | 内存池化、分布式功能、NUMA 资源管理等测�?                 |
| **总计**   | **109** | 多组件集成测�?                                 |

### 核心能力

- **测试框架**: 提供 TestCase �?TestRunner 等核心测试组件，兼容 legacy UniAutos 框架
- **环境管理**: 自动化的测试环境设置和清理，支持 setup/teardown 钩子
- **节点连接**: 通过 SSH 远程执行测试命令，支�?paramiko 库的安全连接
- **配置管理**: 支持 JSON/XML 格式的资源配置文件，灵活定义测试节点信息
- **日志追踪**: 完整的测试步骤日志记录，支持 logStep、logInfo 等日志方�?
- **断言验证**: 内置 assertTrue、assertEqual 等断言方法，支持自动化测试验证
- **并行执行**: 支持 pytest-xdist 并行测试执行，提升测试效�?
- **测试标记**: 支持多种测试标记（unit、integration、virt、container、slow）灵活筛�?
...
- **操作系统**: Linux（推�?openEuler 22.03 LTS 或更高版本）
- **Python 版本**: Python 3.9+（支�?3.9�?.10�?.11�?.12�?.13�?
- **网络要求**: 能够 SSH 连接到测试节�?
...
测试用例执行需要配置测试节点信息，项目支持 JSON �?XML 两种配置格式�?
...
| ip       | �? | 节点 IP 地址          |
| port     | �? | SSH 端口（默�?22�?    |
| user     | �? | SSH 用户名（通常�?root�?|
| password | �? | SSH 密码            |
| nodeId   | �? | 节点标识�?            |
| localIP  | �? | 本地 IP（用于多网卡场景�?   |
| params   | �? | 节点特定参数（键值对字典�?    |

#### 2. XML 配置文件格式（兼�?Legacy�?
...
| virt        | 虚拟化测�?| `pytest -m virt`        |
| container   | 容器测试  | `pytest -m container`   |
| slow        | 慢速测�? | `pytest -m slow`        |
...
```bash
# 使用 pytest-xdist 并行执行（自动检�?CPU 核数�?
...
- `hook` �?(可�? hook 类全限定名，�?`libs.modules.ubsmem.ubsshmem.ubs_mem_hook.UbsMemHook`。`run_suite.py` 自动将其传递为 `--test-hook`
- `tests` �?用例脚本列表（相对于项目根目录的路径�?
- `params` �?统一入参，对�?`--test-params`，测试中通过 `custom_params` fixture 读取，hook �?`_init_from_fixture()` 也会接收此参�?
...
#### 5.1. Hook 机制 �?测试套前�?后置操作

测试套可声明一�?**hook �?*，在用例执行前自动完成环境准备（安装软件、修改配置�?
启动服务等），在用例执行后执行清理�?

**启用 Hook 的条件：**

1. **Suite JSON** 中声�?`hook` 字段（全限定类名�?
2. **测试包目�?* 中存�?`conftest.py`，导入框�?fixture�?
...
3. **Hook �?* 实现以下三个方法即可�?*无需继承特定基类**�?

   ```python
   class MyHook:
       """Hook 类。注意：属性应�?_init_from_fixture() 中设置，而非 __init__�?""

        def _init_from_fixture(self, nodes, custom_params):
            """Fixture 注入入口。框架传�?SSH 节点列表�?test-params 字典�?
   ...
   ```
    ...
    - `_init_from_fixture(nodes, custom_params)` �?接收 `--resource-config` 构建�?`libs.host.Linux` 节点列表�?`--test-params` JSON 字典
   - `beforePreTestSet()` �?前置操作
   - `afterPostTestSet()` �?后置操作
   - 三个方法**缺一不可**；`_init_from_fixture` 被框架识别后会自动调用，即使为空也需定义

   > 💡 如果 hook 逻辑较复杂需要日志、`sleep()` 等辅助方法，可继�?`libs.modules.ubsmem.common.mem_hook_base.MemHookBase`（提�?`self.logger`、`self.sleep()` 等），但**非强�?*�?
...
run_suite.py 读取 hook 字段 �?--test-hook libs.xxx.MyHook
    ...
    �?  ├─ importlib 动态加�?hook �?
    �?  ├─ �?--resource-config �?构建 Linux 节点列表
    �?  ├─ �?--test-params �?custom_params 字典
...
1. **类名不符合pytest命名规范**：`python_classes = ["Test*"]` 要求类名�?`Test` 开�?
2. **缺失依赖�?*：如 `pandas` 未安装导致导入失�?
...
# 如果类名�?"memory_pooling_first_return_success_1"（不以Test开头）
...
3. 参�?`libs/core/basecase/rackcontrol/cm_basecase.py` 的正确模�?
...
本项目采�?MulanPSL2 许可�?- 详见 [LICENSE](LICENSE) 文件�?
