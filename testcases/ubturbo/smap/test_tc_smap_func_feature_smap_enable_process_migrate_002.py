...
class TestTcSmapFuncFeatureSmapEnableProcessMigrate002(SmapCase):
    """
    CaseNumber:
        TC_SMAP_FUNC_FEATURE_SMAP_ENABLE_PROCESS_MIGRATE_002
    RunLevel:
        Level 0
    EnvType:

    CaseName:
        正常调用SmapEnableProcessMigrate禁用冷热迁移，内存迁出失败
    PreCondition:
        P1.OS正常运行
        P2.远端借用内存已上线
        P3.SMAP组件初始化成功
    ...
    TestStep:
        S1.启动 redis-server
        S2.调用SmapMigrateOut redis_pid, 0 将redis 纳入smap管理
        S3.调用SmapEnableProcessMigrate redis_pid 1 1 0 禁用冷热迁移
    ...
    ExpectedResult:
        ...
        E4.远端可用内存设置成功
        E5.迁出配置 设置成功
        E6.能观测到redis-server进程在远端numa上的内存始终为0
    """
    ...
    def test_tc_smap_func_feature_smap_enable_process_migrate_002(self):
        ...
        self.logStep("S2.调用SmapMigrateOut redis_pid, 0 将redis 纳入smap管理")
        rc = self.cli[0].smap_mig_out(MigrateOutMsg([MigrateOutPayload(self.remote_numa_list[0], redis_pids[0], 0)]), 0)
        self.logStep("E2.接口调用成功")
        self.assertEqual(rc, 0)

        self.logStep("S3.调用SmapEnableProcessMigrate redis_pid 1 1 0 禁用冷热迁移")
        rc = self.cli[0].smap_enable_process_migrate([redis_pids[0]], 1, 0, 1)
        self.logStep("E3.禁用冷热迁移设置成功")
        self.assertEqual(rc, 0)
        ...
        self.logStep("S6.通过numastat -p redis_pid 观察 redis 内存使用情况")
        time.sleep(10)
        mem_topo_total = self.hosts[0].get_proc_mem_topo_total(redis_pids[0])
        self.logStep("E6.能观测到redis-server进程 在远端numa上的内存占用始终为0")
    ...
    def teardown_method(self):
        ...
        for redis_pid in self.hosts[0].get_process_id("redis-server"):
            self.cli[0].smap_enable_process_migrate([redis_pid], 1, 1, 1)
