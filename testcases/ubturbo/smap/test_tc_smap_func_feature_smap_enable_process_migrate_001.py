...
class TestTcSmapFuncFeatureSmapEnableProcessMigrate001(SmapCase):
    """
    CaseNumber:
        TC_SMAP_FUNC_FEATURE_SMAP_ENABLE_PROCESS_MIGRATE_001
    RunLevel:
        Level 0
    EnvType:

    CaseName:
        正常调用SmapEnableProcessMigrate启动冷热迁移，内存迁出成功
    PreCondition:
        P1.OS正常运行
        P2.远端借用内存已上线
        P3.SMAP组件初始化成功
    ...
    TestStep:
        S1.启动 redis-server
        S2.调用SmapMigrateOut redis_pid, 0 将redis 纳入smap管理列表
        S3.调用SmapEnableProcessMigrate redis_pid 1 1 1 启动冷热迁移
    ...
    ExpectedResult:
        ...
        E4.远端可用内存设置成功
        E5.迁出配置 设置成功
        E6.能观测到redis-server进程在远端numa上的内存占用增加
    """
    ...
    def test_tc_smap_func_feature_smap_enable_process_migrate_001(self):
        ...
        self.logStep("S2.调用SmapMigrateOut redis_pid, 0 将redis 纳入smap管理列表")
        ...
        self.logStep("S3.调用SmapEnableProcessMigrate redis_pid 1 1 1 启用冷热迁移")
        rc = self.cli[0].smap_enable_process_migrate([redis_pids[0]], 1, 1, 1)
        self.logStep("E3.启用冷热迁移设置成功")
        self.assertEqual(rc, 0)
        ...
        self.logStep("E6.能观测到redis-server进程 在远端numa上的内存占用增加")
