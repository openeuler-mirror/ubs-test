...
class TestTcSmapFuncFeatureSetSmapRemoteNumaInfo002(SmapCase):
    """
    CaseNumber:
        TC_SMAP_FUNC_FEATURE_SET_SMAP_REMOTE_NUMA_INFO_002
    RunLevel:
        Level 0
    EnvType:

    CaseName:
        正常调用SetSmapRemoteNumaInfo接口，绑定单numa
    PreCondition:
        P1.OS正常运行
        P2.远端借用内存已上线
        P3.SMAP组件初始化成功
    TestStep:
        S1.启动 redis-server 进程绑定单numa
        S2.执行redis-benchmark 加压 redis
        S3.通过numastat -p redis_pid 观察 redis 内存使用情况
        S4.调用SetSmapRemoteNumaInfo src_nid, dest_nid, 256M 设置远端可用内存
        S5.调用SmapMigrateOut redis_pid， 100 设置迁出比例为 100%
        S6.通过numastat -p redis_pid 观察远端内存使用情况
    ExpectedResult:
        E1.redis-server 进程启动成功
        E2.redis-benchmark 启动成功
        E3.能观测到redis-server进程 在src_nid上有内存占用
        E4.远端可用内存设置成功
        E5.迁出配置 设置成功
        E6.能观测到redis-server在远端numa上的内存占用增加且占用量始终小于256M，在src_nid上的占用减少
    """

    def setup_method(self):
        super(TestTcSmapFuncFeatureSetSmapRemoteNumaInfo002, self).preTestCase()
    ...
    def test_tc_smap_func_feature_set_smap_remote_numa_info_002(self):
        self.logStep("S1.启动 redis-server 进程绑定单numa")
        result = self.hosts[0].start_redis_with_numa_nodes([self.local_numa_list[0]], 6379)
        ...
        result = self.hosts[0].watch_proc_mem(redis_pids[0], self.local_numa_list[0], 512, 180)
        self.logStep("E3.能观测到redis-server进程 在src_nid上有内存占用")
        self.assertEqual(result, True)
        mem_topo = self.hosts[0].get_proc_mem_topo_total(redis_pids[0])

        self.logStep("S4.调用SetSmapRemoteNumaInfo src_nid, dest_nid, 256M 设置远端可用内存")
        ...
        self.assertEqual(result, True)
        self.logStep("E6.能观测到redis-server在远端numa上的内存占用增加且占用量始终小于256M，在 src_nid 上的占用减少")
        sleep(10)
    ...
    def teardown_method(self):
        super(TestTcSmapFuncFeatureSetSmapRemoteNumaInfo002, self).postTestCase()
