...
class TestTcSmapFuncFeatureSetRunMode001(SmapCase):
    """
    CaseNumber:
        TC_SMAP_FUNC_FEATURE_SET_RUN_MODE_001
    RunLevel:
        Level 0
    EnvType:

    CaseName:
        正常调用SetSmapRunMode设置运行模式为水线模式，通过配置迁出比例迁出内存成功
    PreCondition:
        P1.OS正常运行
        P2.远端借用内存已上线
        P3.SMAP组件初始化成功
    ...
    TestStep:
        S1.启动 redis-server
        S2.调用SetSmapRunMode 0 将运行模式设置为水线模式
    ...
    def test_tc_smap_func_feature_set_run_mode_001(self):
        ...
        self.logStep("S2.调用SetSmapRunMode 0 将运行模式设置为水线模式")
        rc = self.cli[0].smap_set_runmode(0)
    ...
    def teardown_method(self):
        if len(self.local_numa_list) == 0 or len(self.remote_numa_list) < 1:
            return
