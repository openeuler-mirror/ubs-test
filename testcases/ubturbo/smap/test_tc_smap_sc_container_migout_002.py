...
class TestTcSmapScContainerMigout002(SmapCase):
    """
    CaseNumber:
        TC_SMAP_SC_CONTAINER_MIGOUT_002
    RunLevel:
        Level 1
    EnvType:

    CaseName:
        002容器内进程绑定单numa，迁出后更换远端numa
    PreCondition:
        P1.OS正常运行
        P2.容器正常运行
        P3.远端借用内存已上线
    ...
    TestStep:
        ...
        S7.调用SmapMigratePidRemoteNuma redis_pid, 1, dest_nid, dest_nid_new 更换远端numa
    ...
    def setup_method(self):
        super(TestTcSmapScContainerMigout002, self).preTestCase()
    ...
    def test_tc_smap_sc_container_migout_002(self):
        ...
        result = self.hosts[0].start_redis_benchmark_full_param(
            10 ** 11, 128, 64 * 1024, 8 * 1024, 8, self.hosts[0].containers[0].get_container_ip(), 6379)
        ...
        rc = self.cli[0].smap_enable_process_migrate([int(redis_pids[0])], 1, 0, 0)
        self.assertEqual(rc, 0)
        ...
        msg = MigratePidNumaMsg(
            count=1,
            payload=[
                MigratePidNumaPayload(int(redis_pids[0]), self.remote_numa_list[0], self.remote_numa_list[1], 25, 0)]
        )
        rc = self.cli[0].smap_migrate_pid_remote_numa(msg, 0)
        self.assertEqual(rc, 0)
        rc = self.cli[0].smap_enable_process_migrate([int(redis_pids[0])], 1, 1, 0)
        ...
        self.assertEqual(result, True)
        result = self.hosts[0].watch_proc_mem(redis_pids[0], self.remote_numa_list[1], 128, 180)
        self.assertEqual(result, True)
    ...
    def teardown_method(self):
        super(TestTcSmapScContainerMigout002, self).postTestCase()
