#!/usr/local/python
# -*- coding: utf-8 -*-
# Copyright (c) Huawei Technologies Co., Ltd. 2025-2025. All rights reserved


from libs.core.basecase.ubturbo import SmapCase, MigrateOutMsg, MigrateOutPayload, EnableNodeMsg, RemoveMsg


class TestTcSmapFuncFeatureSmapMigrateOut002(SmapCase):
    """
    CaseNumber:
        TC_SMAP_FUNC_FEATURE_SMAP_MIGRATE_OUT_002
    RunLevel:
        Level 0
    EnvType:

    CaseName:
        正常调用SmapMigrateOut迁出虚机进程，内存迁出成功
    PreCondition:
        P1.OS正常运行
        P2.远端借用内存已上线
        P3.SMAP组件初始化成功
        P4.远端numa使能成功
    TestStep:
        S1.执行virsh create xxx.xml启动4U8G虚拟机
        S2.调用SetSmapRemoteNumaInfo src_nid dest_nid, 4G 设置远端可用内存
        S3.调用SmapMigrateOut vm_pid, 25 设置迁出比例为 25%
        S4.通过numastat -p redis_pid 观察redis内存使用情况
        S5.调用SmapMigrateOut vm_pid, 0 触发内存迁回
        S6.通过numastat -p redis_pid 观察redis内存使用情况
    ExpectedResult:
        E1.虚拟机启动成功
        E2.远端可用内存设置成功
        E3.迁出配置 设置成功
        E4.能观测到虚机进程dest_nid 上的内存占用增加
        E5.接口调用成功
        E6.能观测到虚机进程dest_nid 上的内存占用清零
    """

    def setup_method(self):
        self.logStep("P1.OS正常运行")
        self.local_numa_list = self.hosts[0].get_local_numa()
        self.assertGreaterEqual(len(self.local_numa_list), 1)

        self.logStep("P2.远端借用内存已上线")
        self.remote_numa_list = self.hosts[0].get_remote_numa()
        self.assertGreaterEqual(len(self.remote_numa_list), 1)

        self.logStep("P3.SMAP组件初始化成功")
        rc = self.cli[0].smap_init(1)
        self.assertEqual(rc in (0, -1), True)

        self.logStep("P4.远端numa使能成功")
        rc = self.cli[0].smap_enable_node(EnableNodeMsg(1, self.remote_numa_list[0]))
        self.assertEqual(rc, 0)

    def test_tc_smap_func_feature_smap_migrate_out_002(self):
        self.logStep("S1.执行virsh create xxx.xml启动4U8G虚拟机")
        result = self.hosts[0].vm_nodes[0].create()
        self.logStep("E1.虚拟机启动成功")
        self.assertEqual(result, True)
        vm_pid = self.hosts[0].vm_nodes[0].get_pid()
        self.assertNotEqual(vm_pid, -1)
        vm_numa = self.hosts[0].vm_nodes[0].get_unique_numa_node()
        mem_topo = self.hosts[0].get_proc_mem_topo_total(vm_pid)

        self.logStep("S2.调用SetSmapRemoteNumaInfo src_nid dest_nid, 4G 设置远端可用内存")
        rc = self.cli[0].set_smap_remote_numa_info(vm_numa, self.remote_numa_list[0], 4096)
        self.logStep("E2.远端可用内存设置成功")
        self.assertEqual(rc, 0)

        self.logStep("S3.调用SmapMigrateOut vm_pid, 25 设置迁出比例为 25%")
        rc = self.cli[0].smap_mig_out(MigrateOutMsg([MigrateOutPayload(self.remote_numa_list[0], vm_pid, 25)]), 1)
        self.logStep("E3.迁出配置 设置成功")
        self.assertEqual(rc, 0)

        self.logStep("S4.通过numastat -p redis_pid 观察redis内存使用情况")
        result = self.hosts[0].watch_proc_mem(vm_pid, self.remote_numa_list[0], 512, 180)
        self.logStep("E4.能观测到虚拟机进程dest_nid 上的内存占用增加")
        self.assertEqual(result, True)
        for nid, mem in enumerate(self.hosts[0].get_proc_mem_topo_total(vm_pid)):
            if nid == self.remote_numa_list[0]:
                self.assertEqual(mem > mem_topo[nid], True)
            mem_topo[nid] = mem

        self.logStep("S5.调用SmapMigrateOut vm_pid, 0 触发内存迁回")
        rc = self.cli[0].smap_mig_out(MigrateOutMsg([MigrateOutPayload(self.remote_numa_list[0], vm_pid, 0)]), 1)
        self.logStep("E5.接口调用成功")
        self.assertEqual(rc, 0)

        self.logStep("S6.通过numastat -p redis_pid 观察redis内存使用情况")
        result = self.hosts[0].watch_proc_mem(vm_pid, self.remote_numa_list[0], 0, 180, True)
        self.logStep("E6.能观测到虚拟机进程dest_nid 上的内存占用清零")
        self.assertEqual(result, True)
        for nid, mem in enumerate(self.hosts[0].get_proc_mem_topo_total(vm_pid)):
            if nid == self.remote_numa_list[0]:
                self.assertEqual(mem == 0, True)
            mem_topo[nid] = mem

    def teardown_method(self):
        if len(self.remote_numa_list) == 0 or len(self.local_numa_list) == 0:
            return
        vm_pid = self.hosts[0].vm_nodes[0].get_pid()
        if vm_pid != -1:
            return
        vm_numa = self.hosts[0].vm_nodes[0].get_unique_numa_node()
        self.cli[0].set_smap_remote_numa_info(vm_numa, self.remote_numa_list[0], 0)
        self.hosts[0].watch_proc_mem(vm_pid, self.remote_numa_list[0], 0, 180, True)
        self.cli[0].smap_remove(RemoveMsg([vm_pid]), 1)