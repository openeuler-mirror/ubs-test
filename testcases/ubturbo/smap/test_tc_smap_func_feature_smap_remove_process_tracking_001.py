...
class TestTcSmapFuncFeatureSmapRemoveProcessTracking001(SmapCase):
    """
    CaseNumber:
        TC_SMAP_FUNC_FEATURE_SMAP_REMOVE_PROCESS_TRACKING_001
    RunLevel:
        Level 0
    EnvType:
    ...
    TestStep:
        ...
        S2.1）执行smap smap_remove vm_pid 1 确保虚机进程未被SMAP纳管
        S2.2）执行smap smap_add_process_tracking pid_str pid_time 1 0 将进程设置为只扫描状态
        S3.检查 /proc/{PID}_t/tracking_info 是否存在
        S4.调用 smap smap_query_freq_info vm_pid 读取/proc/{PID}_t/tracking_info 文件
        S5.执行smap smap_remove_process_tracking vm_pid 1 0 通知SMAP移除进程扫描
        S6.检查 /proc/{PID}_t/tracking_info 是否存在
    ExpectedResult:
        E1.虚拟机启动成功
        E2.添加扫描模式成功
        ...
        E5.移除扫描模式成功
        E6./proc/{PID}_t/tracking_info 不存在
    """

    def setup_method(self):
        super(TestTcSmapFuncFeatureSmapRemoveProcessTracking001, self).preTestCase()
    ...
    def test_tc_smap_func_feature_smap_remove_process_tracking_001(self):
        ...
        self.logStep("S2.1）执行smap smap_remove vm_pid 1 确保虚机进程未被SMAP纳管")
        self.logStep("S2.2）执行smap smap_add_process_tracking pid_str pid_time 1 0 将进程设置为只扫描状态")
        ...
        self.logStep("S5.执行smap smap_remove_process_tracking vm_pid 1 0 通知SMAP移除进程扫描")
        self.logStep("E5.移除扫描模式成功")
        rc = self.cli[0].smap_remove_process_tracking([vm_pid], 1, 0)
        self.assertEqual(rc, 0)

        self.logStep("S6.检查 /proc/{PID}_t/tracking_info 是否存在")
        self.logStep("E6./proc/{PID}_t/tracking_info 不存在")
        result = self.hosts[0].check_file_exists(f"/proc/{vm_pid}_t/tracking_info")
        self.assertEqual(result, False)

    def teardown_method(self):
        super(TestTcSmapFuncFeatureSmapRemoveProcessTracking001, self).postTestCase()
