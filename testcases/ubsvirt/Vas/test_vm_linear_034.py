...
class TestVmLinear034(VasBaseCase):
    """验证超分场景下动态绑定虚机重调度尽可能少跨Cluster.
    ...
    TestStep:
        ...
        S2.执行vasctl opt reassign --scope all，后执行vasctl query affinity --scope all查看虚机绑核情况
    ExpectedResult:
    ...
    def test_vm_linear_034(self):
        ...
        self.logStep("E1.删除成功，VM3绑核信息没有变化，没有碎片整理，VM2碎片整理，绑定变化成绑cpu2-9")
        ...
        self.logStep("S2.执行vasctl opt reassign --scope all，后执行vasctl query affinity --scope all查看虚机绑核情况")
        ...
        time.sleep(2)
        res1 = self.check_query_affinity("VM3", self.cluster_size, self.cluster_size * 2 - 1)
        res2 = self.check_query_affinity("VM2", 2, self.cluster_size - 1)

        self.logStep("E2.VM3绑核信息变化。变成16-31")
        self.assertTrue(res1, "The vm3's bound vCPUs are not in cluster_size - (cluster_size * 2 - 1).")
        self.assertTrue(res2, "The vm2's bound vCPUs are not in 2 - (cluster_size - 1).")
