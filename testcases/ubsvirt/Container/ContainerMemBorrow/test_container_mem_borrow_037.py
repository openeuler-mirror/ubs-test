...
@pytest.mark.smoke
class TestContainerMemBorrow037(KubernetesBaseCase):
    """验证不绑numa容器加压超水线后内存借用成功.

    CaseNumber:
        test_container_mem_borrow_037
    RunLevel:
        Level 1
    EnvType:
    ...
    TestStep:
        S1.创建pod
        S2.登录pod，清理缓存，使用大页占用小页内存，4个numa共预留15.5G
        ...
        S5.在master节点使用kubectl get event查询借用事件，使用numastat -vm查询借用内存，借用值为4G
        S6.删除pod，清理环境
    ExpectedResult:
        E1.pod创建成功，没有报错
    ...
    def init_mem_borrow_params(self):
        ...
        base_path = Path(__file__).parent.parent.parent.parent.parent / "resource" / "ubsvirt" / "yaml" / "test_container_mem_borrow_037"
    ...
    def setup_method(self):
        ...
        params = {
            "source_file": f"{self.yaml_base_path}/pod-node-worker1.yaml",
            "destination_file": f"{self.filepath}/pod-node-worker1.yaml"
        }
        self.upload_file("master", params)

    def test_container_mem_borrow_037(self):
        """测试不绑numa容器加压超水线后内存借用成功"""

        self.logStep("S1.创建pod")
        node = self.node_dict[self.run_node_name]
        self.delete_pod_by_name(self.pod_name)
        create_result = self.create_pod_by_name("pod-node-worker1.yaml")
        ...
        self.logStep("S2.登录pod，清理缓存，使用大页占用小页内存，4个numa共预留15.5G")
