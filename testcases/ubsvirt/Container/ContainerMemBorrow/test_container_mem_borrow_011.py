...
@pytest.mark.smoke
class TestContainerMemBorrow011(KubernetesBaseCase):
    """验证numa级内存监控服务生成水线告警.

    CaseNumber:
        test_container_mem_borrow_011
    RunLevel:
        Level 1
    EnvType:
    ...
    PreCondition:
        P1.环境中存在2个及以上节点
        P2.K8s/MXE功能正常无异常
        P3.环境中已部署pod和容器，容器limit设置numa可用小页内存（可通过配置大页数来构造对应小页内存大小）
    TestStep:
    ...
    def init_mem_borrow_params(self):
        ...
        self.yaml_base_path = Path(__file__).parent.parent.parent.parent.parent / "resource" / "ubsvirt" / "yaml" / "test_container_mem_borrow_011"
    ...
    def setup_method(self):
        ...
        self.logStep("P3.环境中已部署pod和容器，容器limit设置numa可用小页内存")
        self.set_label_numa()
    ...
    def test_container_mem_borrow_011(self):
        ...
        numa_num = self.get_node_container_numa_affinity_by_name(self.run_node_name, self.pod_name)
