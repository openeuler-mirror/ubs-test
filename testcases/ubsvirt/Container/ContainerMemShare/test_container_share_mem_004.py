...
@pytest.mark.smoke
class TestContainerShareMem004(KubernetesBaseCase):
    """验证同个pod创建多个共享内存后资源超出限制创建失败.

    CaseNumber:
        test_container_share_mem_004
    RunLevel:
        Level T
    EnvType:
    ...
    TestStep:
        ...
        app ubsm_shmem_allocate default sharememory_2 1073741824 777 0
        S4.映射共享内存
        例如 app ubsm_shmem_map 0 1073741824 3 1 sharememory_1 0 1
        app ubsm_shmem_map 0 1073741824 3 1 sharememory_2 0 1
        S5.再次创建1个1024MB共享内存
        例如 app ubsm_shmem_map 0 1073741824 3 1 sharememory_3 0 1
        S6.查看映射目录，查看CR文件的Status
        S7.解除映射
    ...
    ExpectedResult:
        ...
        E3.分别返回ubsm_shmem_allocate ret(0)，代表映射成功
        E4.分别返回ubsm_shmem_allocate addr 0xffff10000000 ,ret(0)。地址为随机值
        E5.创建失败，报错资源不足，超过CR文件配置限制额度
        E6.CR文件更新，已经记录当前已使用的共享内存的数据
    ...
    def init_shm_params(self):
        ...
        self.shm_name1 = "sharememory_1"
        self.shm_name2 = "sharememory_2"
        self.shm_name3 = "sharememory_3"
        self.cr_name = self.shm_name1.split("_")[0]
        self.filepath = "/tmp/memborrow"
        base_path = Path(__file__).parent.parent.parent.parent.parent / "resource" / "ubsvirt" / "yaml" / "test_container_share_mem_004"
    ...
    def test_container_share_mem_004(self):
        ...
        self.logStep("S3.创建2个1024MB共享内存"
                     "例如 app ubsm_shmem_allocate default sharememory_1 1073741824 777 0"
                     "app ubsm_shmem_allocate default sharememory_2 1073741824 777 0")
        cmd_result1 = self.allocate_shm(self.pod_name, self.shm_name1, self.shm_size)
        cmd_result2 = self.allocate_shm(self.pod_name, self.shm_name2, self.shm_size)
        cr_result = self.get_cr_status(self.cr_name)

        self.logStep("E3.分别返回ubsm_shmem_allocate ret(0)，代表映射成功")
        self.logInfo(f"allocate_shm cmd_result {cmd_result1}")
        self.assertTrue(cmd_result1, "映射失败")
        self.assertTrue(self.get_key_from_result(cr_result, self.shm_name1), "CR文件未更新")
        self.logInfo(f"allocate_shm cmd_result {cmd_result2}")
        self.assertTrue(cmd_result2, "映射失败")
        self.assertTrue(self.get_key_from_result(cr_result, self.shm_name2), "CR文件未更新")

        self.logStep("S4.映射共享内存"
                     "例如 app ubsm_shmem_map 0 1073741824 3 1 sharememory_1 0 1"
                     "app ubsm_shmem_map 0 1073741824 3 1 sharememory_2 0 1")
        cmd_result1 = self.map_shm(self.pod_name, self.shm_size, "0", "1", self.shm_name1)
        cmd_result2 = self.map_shm(self.pod_name, self.shm_size, "0", "1", self.shm_name2)

        self.logStep("E4.分别返回ubsm_shmem_allocate addr 0xffff10000000 ,ret(0)。地址为随机值")
        self.assertTrue(cmd_result1, "映射共享内存失败，未获取到映射地址")
        self.assertTrue(cmd_result2, "映射共享内存失败，未获取到映射地址")

        self.logStep("S5.再次创建1个1024MB共享内存"
                     "例如 app ubsm_shmem_map 0 1073741824 3 1 sharememory_3 0 1")
        cmd_result = self.allocate_shm(self.pod_name, self.shm_name3, self.shm_size)
        self.logInfo(f"allocate_shm cmd_result {cmd_result}")

        self.logStep("E5.创建失败，报错资源不足，超过CR文件配置限制额度")
        self.assertFalse(cmd_result, "映射成功，但是不应该")

        self.logStep("S6.查看映射目录，查看CR文件的Status，是否记录当前已使用的共享内存的数据")
        cr_result = self.get_cr_status(self.cr_name)

        self.logStep("E6.CR文件更新，已经记录当前已使用的共享内存的数据")
        self.assertTrue(self.get_key_from_result(cr_result, self.shm_name1), "CR文件更新了")
        self.assertTrue(self.get_key_from_result(cr_result, self.shm_name2), "CR文件更新了")
        self.assertFalse(self.get_key_from_result(cr_result, self.shm_name3), "CR文件更新了")

        self.logStep("S7.解除映射"
                     "例如 app ubsm_shmem_unmap 0xffff10000000 1073741824")
        self.logInfo("自动化工具中会自动解除映射")

        self.logStep("E7.返回ubsm_shmem_unmap  ret(0)")
        self.assertEqual(self.get_shm_dev_nums(self.get_tmp_dev_num(self.pod_name)), 0, "映射的文件数量不正确")

        self.logStep("S8.删除共享内存"
                     "例如 app ubsm_shmem_deallocate sharememory_1")
        cmd_result1 = self.deallocate_shm(self.pod_name, self.shm_name1)
        cmd_result2 = self.deallocate_shm(self.pod_name, self.shm_name2)

        self.logStep("E8.返回ubsm_shmem_deallocate ret(0)")
        self.assertTrue(cmd_result1, "删除共享内存失败")
        self.assertTrue(cmd_result2, "删除共享内存失败")

        self.logStep("S10.查看CR文件的Status，是否删除CR的status相应的name")
        cr_result = self.get_cr_status(self.cr_name)

        self.logStep("E9.查看CR文件的Status，已经删除CR的status相应的name")
        self.assertFalse(self.get_key_from_result(cr_result, self.shm_name1), "CR文件更新了")
        self.assertFalse(self.get_key_from_result(cr_result, self.shm_name2), "CR文件更新了")

    def teardown_method(self):
        self.deallocate_shm(self.pod_name, self.shm_name1)
        self.deallocate_shm(self.pod_name, self.shm_name2)
        time.sleep(5)
        self.delete_pod_and_wait(self.pod_name)
