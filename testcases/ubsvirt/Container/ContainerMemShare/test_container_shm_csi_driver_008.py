...
@pytest.mark.smoke
class TestContainerShmCsiDriver008(KubernetesBaseCase):
    """验证shm-csi-driver异常退出自动恢复建链后可以创建映射内存.

    CaseNumber:
        test_container_shm_csi_driver_008
    RunLevel:
        Level 1
    EnvType:
    ...
    TestStep:
        ...
        S5.构造matrix-csi-driver进程退出故障，映射共享内存
        S6.等待一分钟查看matrix-csi-driver进程状态
        S7.映射共享内存
    ...
    ExpectedResult:
        ...
        E3.成功进入接口输入页面
        E4.创建共享内存成功
        E5.构造成功，matrix-csi-driver进程挂掉，映射共享内存失败
        E6.matrix-csi-driver进程状态可以恢复正常
    ...
    def init_shm_params(self):
        ...
        base_path = Path(__file__).parent.parent.parent.parent.parent / "resource" / "ubsvirt" / "yaml" / "test_container_shm_csi_driver_008"
    ...
    def test_container_shm_csi_driver_008(self):
        ...
        self.logStep("E5.构造成功，matrix-csi-driver进程挂掉，映射共享内存失败")

        self.logStep("S6.等待一分钟查看matrix-csi-driver进程状态")
        self.restart_shm_pod()
        pod_list = self.get_pod_list_by_name("shm-csi-driver")
        self.assertEqual(len(pod_list), self.get_node_number(), "pod数为0")

        self.logStep("E6.matrix-csi-driver进程状态可以恢复正常")
