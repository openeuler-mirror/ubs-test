...
@pytest.mark.smoke
class TestContainerShmCsiDriver002(KubernetesBaseCase):
    """验证shm-csi-driver异常退出自动恢复建链后可以归还映射内存.

    CaseNumber:
        test_container_shm_csi_driver_002
    RunLevel:
        Level 1
    EnvType:
    ...
    TestStep:
        ...
        S6.构造matrix-csi-driver进程退出故障
        S7.解除映射
        例如 app ubsm_shmem_unmap 0xffff10000000 1073741824
        S8.等待一分钟查看matrix-csi-driver进程状态，再次解除映射
        S9.构造matrix-csi-driver进程退出故障
        S10.删除共享内存
        例如 app ubsm_shmem_deallocate sharememory_1
        S11.等待一分钟查看matrix-csi-driver进程状态，再次删除共享内存
    ExpectedResult:
        E1.pod创建成功，没有报错
        ...
        E6.构造成功，matrix-csi-driver进程挂掉
        E7.解除共享内存映射失败
        E8.matrix-csi-driver进程状态可以恢复正常，解除映射成功，返回ubsm_shmem_unmap  ret(0)
        E9.构造成功，matrix-csi-driver进程挂掉
        E10.删除共享内存失败
    ...
    def init_shm_params(self):
        ...
        base_path = Path(__file__).parent.parent.parent.parent.parent / "resource" / "ubsvirt" / "yaml" / "test_container_shm_csi_driver_002"
    ...
    def test_container_shm_csi_driver_002(self):
        ...
        self.logStep("S4.再次创建1024MB共享内存"
                     "例如 app ubsm_shmem_allocate default sharememory_1 1073741824 777 0")
        ...
        self.logStep("S6.构造matrix-csi-driver进程退出故障")
        self.stop_shm_pod()

        self.logStep("E6.构造成功，matrix-csi-driver进程挂掉")
        ...
        self.logStep("E7.解除共享内存映射失败")

        self.logStep("S8.等待一分钟查看matrix-csi-driver进程状态，再次解除映射")
        self.restart_shm_pod()
        cmd_result4 = self.unmap_shm(self.pod_name1, self.shm_size, "0", "1", self.shm_name1)
        self.assertTrue(self.get_key_from_result(cmd_result4, "ptr"), "映射共享内存失败，未获取到映射地址")

        self.logStep("E8.matrix-csi-driver进程状态可以恢复正常，解除映射成功，返回ubsm_shmem_unmap  ret(0)")

        self.logStep("S9.构造matrix-csi-driver进程退出故障")
        self.stop_shm_pod()
        pod_list = self.get_pod_list_by_name("shm-csi-driver")
        self.assertEqual(len(pod_list), 0, "pod数不为0")

        self.logStep("E9.构造成功，matrix-csi-driver进程挂掉")

        self.logStep("S10.删除共享内存"
                     "例如 app ubsm_shmem_deallocate sharememory_1")
        cmd_result5 = self.deallocate_shm(self.pod_name1, self.shm_name1)
        self.assertFalse(cmd_result5, "预期删除共享内存失败")

        self.logStep("E10.删除共享内存失败")

        self.logStep("S11.等待一分钟查看matrix-csi-driver进程状态，再次删除共享内存"
                     "例如 app ubsm_shmem_deallocate sharememory_1")
        self.restart_shm_pod()
        cmd_result6 = self.deallocate_shm(self.pod_name1, self.shm_name1)
        self.assertTrue(cmd_result6, "删除共享内存失败")

        self.logStep("E11.matrix-csi-driver进程状态可以恢复正常，删除内存成功，返回ubsm_shmem_deallocate ret(0)")
