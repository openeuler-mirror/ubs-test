...
class TestVmMigrateXml001(VMxmlBaseCase):
    ...
    CaseNumber:
        test_vm_migrate_xml_001
    RunLevel:
        Level 0
    PreCondition:
        P1、环境中存在2个及以上节点
        P2、libvirt和qemu组件正常部署
        P3、环境上存在2U4G虚机xml和相应虚机镜像
        P4、节点1和节点2的numa0分别配置5G大页内存
    TestStep:
        S1、使用xml在节点1创建虚机
        S2、登录虚机，使用stress-ng加压80%
        S3、执行虚机迁移virsh migrate --p2p --live --unsafe --migrateuri hcom://dst_ip vm_name --listen-address dst_ip qemu+tcp://dst_ip/system --verbose --rdma-pin-all --onecopy
    ExpectedResult:
        E1、虚机创建成功
        E2、加压成功，虚机正常运行
        E3、虚机成功迁移到对端
    ...
    def setup_method(self):
        self.vm_name = "test_vm_migrate_xml_001_vm"
    ...
    def teardown_method(self):
        ...
        self.master.run({'command': [f"rm -rf {self.VM_XML_PATH}/test_vm_migrate_xml_001_vm.xml"]})
    ...
    def test_vm_migrate_xml_001(self, xml_base_path):
        ...
        self.logStep("P3、环境上存在2U4G虚机xml和相应虚机镜像")
        ...
        vm_created = self.create_vm_from_xml(
            self.master, str(xml_base_path), self.VM_XML_PATH, "test_vm_migrate_xml_001_vm.xml"
        )
        ...
        self.assertTrue(3072 <= int(vm_mem["used"]), 
                        f"test_vm_migrate_xml_001_vm mem {vm_mem['used']} not in expected range")
        
        self.logStep("S3、执行虚机迁移virsh migrate --p2p --live --unsafe --migrateuri hcom://dst_ip vm_name --listen-address dst_ip qemu+tcp://dst_ip/system --verbose --rdma-pin-all --onecopy")
        migrate_cmd = (
            ...
            f"--verbose --rdma-pin-all --onecopy"
        )
