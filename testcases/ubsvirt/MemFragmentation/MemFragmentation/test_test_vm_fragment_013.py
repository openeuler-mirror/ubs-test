"""
Migrated from legacy: test_vm_fragment_013
"""
...
@pytest.mark.smoke
class TestVmFragment013(OpenStackBaseCase):
    """
    CaseNumber:
        test_vm_fragment_013
    RunLevel:
        Level 1
    EnvType:
        None
    ...
    def test_vm_fragment_013(self, get_topo_path):
        ...
        self.vm_list = self.prepare_topo(str(get_topo_path("test_test_vm_fragment_013")))
        ...
        self.logStep(
            ...
            '决策为本地创建（"should_borrow_mem": False），需要本地已有虚机进行内存迁移的量为0（"migrate_mem": 0）， 远端创建内存量为0（"remote_mem": 0）'
        )
        ...
        server_detail = self.wait_server_target_status(
            "vm_5g", {"status": "ACTIVE", "OS-EXT-SRV-ATTR:host": self.node_dict["node1"].host}
        )
        self.assertEqual(server_detail["OS-EXT-SRV-ATTR:host"], self.node_dict["node1"].host)
