...
class TestUbsVirt004(OpenStackBaseCase):
    """
    CaseNumber:
        test_ubs_virt_004
    RunLevel:
    ...
    CaseName:
        碎片场景，调用/notOverAllocation/migration/hostname获取逃生决策结果成功
    PreCondition:
        P1.环境中UBS Scheduler状态正常
        P2.获取环境中keystone的token信息
        P3.环境为碎片场景
    TestStep:
        S1.调用/notOverAllocation/migration/hostname相关接口，查看响应结果是否满足预期
    ExpectedResult:
    ...
    def test_ubs_virt_004(self, get_topo_path):
        self.logStep("S1.调用/notOverAllocation/migration/hostname相关接口，查看响应结果是否满足预期")
        self.vm_list = self.prepare_topo(str(get_topo_path("test_ubs_virt_004")))
        ...
        query_res = test_api.apitest_migration_hostname(self.controller, vm_id, self.vm_list[0].ram * 1024 * 1024, self.token)

        self.logInfo("E1.接口调用成功，状态码返回200，相应结果符合预期")
        self.assertIn("200", query_res, "return code is not 200")
        self.assertIn(hostname, query_res, f"调用接查询的hostname与预期：{hostname}不一致")