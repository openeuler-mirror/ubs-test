...
class TestVmHotPlugCreate006(VMHotPlugBaseCase):
    ...
    def test_vm_hot_plug_create_006(self, xml_base_path):
        self.logStep("S1、使用xml创建虚拟机vm_01，vm_02")
        vm1_created = self.create_vm_from_xml(
            self.master, str(xml_base_path), self.filepath, "test_vm_hot_plug_create_006_vm_01.xml"
        )
        ...
        vm2_created = self.create_vm_from_xml(
            self.master, str(xml_base_path), self.filepath, "test_vm_hot_plug_create_006_vm_02.xml"
        )
