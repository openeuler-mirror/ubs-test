...
def set_mem_fragmentation(resource_config: dict):
    ...
    basecase.logStep("开始切换为碎片环境")
    basecase.change_overcommitment(openstack_nova_conf, 1)
    basecase.logStep("成功切换为碎片环境")
    basecase.logStep("ubsvirt碎片用例开始执行")

    yield

    basecase.logStep("ubsvirt碎片用例执行结束")