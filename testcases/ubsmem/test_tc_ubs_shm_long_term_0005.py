...
class TestTcUbsShmLongTerm0005(UbsMemCase):
    ...
    CaseName:
        005验证创建单边NC模式不同大小共享内存16进程进行一写多读
    PreCondition:
        P1.UBS-Engine进程正常拉起
        P2.MMI组件加载正常
        P3.UBS-Memory服务加载正常
    ...
    TestStep:
        ...
        S4.进程0按128M、1G、4G执行app ubsm_shmem_allocate region_name shm_name 1024 0600 UBSM_FLAG_ONLY_IMPORT_NONCACHE|UBSM_FLAG_WR_DELAY_COMP(12)创建共享内存
        S5.所有节点的所有进程并发执行app ubsm_shmem_map addr 1024 PROT_READ|PROT_WRITE(3) 0 shm_name 0 1映射进程0创建的共享内存
        S6.所有节点依次循环执行S7-S10
        S7.执行dd if=/dev/urandom of=write_test.txt bs=1M count=size按内存大小生成随机文件并获取md5值
    ...
    def test_tc_ubs_shm_long_term_0005(self):
        ...
        for node in self.host_nodes:
            ...
            while sizes:
                ...
                self.logStep("S5.所有节点的所有进程并发执行app ubsm_shmem_map addr 1024 PROT_READ|PROT_WRITE(3) 0 shm_name 0 1映射进程0创建的共享内存")
                ...
                self.logStep("E6.开始循环执行")
                self.logStep("S7.执行dd if=/dev/urandom of=write_test.txt bs=1M count=size按内存大小生成随机文件并获取md5值")
                ...
                self.logStep("S10.获取文件的md5值并做对比")
                self.logStep("E10.两次md5值一直")
