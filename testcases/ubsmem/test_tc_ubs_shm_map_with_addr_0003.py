...
class TestTcUbsShmMapWithAddr0003(UbsMemCase):
    ...
    TestStep:
        ...
        S6.随机取一个字符并写入得到的地址中
        S7.检查内存内容是否与写入的一致
        S8.调用接口ubsmem_shmem_unmap addr 1024*1024*1024 解除映射的共享内存
        S9.调用接口ubsmem_shmem_deallocate shm_name
        S10.节点0调用接口read addr 1024 test.txt 读取内存的内容到文件
        S11.节点0获取文件的md5值并做对比
        S12.调用接口ubsmem_shmem_unmap addr 1024*1024*1024 解除映射的共享内存
    ...
    def test_tc_ubs_shm_map_with_addr_0003(self):
        ...
        self.logStep("S7.检查内存内容是否与写入的一致")
        rc = self.host_nodes[0].apps[0].mem_check(addr_desc2.addr, size, expect_char)
        self.logStep("E7.内存检查结果一致")
        self.assertEqual(rc, UBSM_SHMEM_OK)

        self.logStep("S8.调用接口ubsmem_shmem_unmap addr 1024*1024*1024 解除映射的共享内存")
