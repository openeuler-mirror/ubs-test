...
@pytest.mark.smoke
class TestTcUbsMemBorrow0007(UbsMemCase):
    """
    CaseNumber:
        TC_UBS_MEM_BORROW_0007
    RunLevel:
        Level 0
    EnvType:

    CaseName:
        007验证借用128M内存后进行先读后写操作
    PreCondition:
        P1.UBS-Engine进程正常拉起
        P2.MMI组件加载正常
        P3.UBS-Memory服务加载正常
    TestStep:
        S1.调用接口ubsmem_lease_malloc region 128*1024*1024 0 0
        S2.随机取一个字符并写入得到的地址中
        S3.检查内存内容是否与写入的一致
    ...
    def test_tc_ubs_mem_borrow_0007(self):
        ...
        self.logStep("S1.调用接口ubsmem_lease_malloc region 128*1024*1024 0 0")
