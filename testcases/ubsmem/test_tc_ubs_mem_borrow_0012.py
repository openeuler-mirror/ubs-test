...
@pytest.mark.smoke
class TestTcUbsMemBorrow0012(UbsMemCase):
    """
    CaseNumber:
        TC_UBS_MEM_BORROW_0012
    RunLevel:
        Level 2
    EnvType:

    CaseName:
        012验证numa借用1G内存后进行先写后读操作
    PreCondition:
        P1.UBS-Engine进程正常拉起
        P2.MMI组件加载正常
        P3.UBS-Memory服务加载正常
    TestStep:
        S1.调用接口ubsmem_lease_malloc name 1024*1024*1024 0 64
        S2.随机取一个字符并写入得到的地址中
        S3.检查内存内容是否与写入的一致
    ...
    def test_tc_ubs_mem_borrow_0012(self):
        size = 1024 * 1024 * 1024
        self.logStep("S1.调用接口ubsmem_lease_malloc name 1024*1024*1024 0 64")
