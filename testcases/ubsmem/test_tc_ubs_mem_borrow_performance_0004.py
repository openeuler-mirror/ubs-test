...
@pytest.mark.smoke
class TestTcUbsMemBorrowPerformance0004(UbsMemCase):
    """
    CaseNumber:
        TC_UBS_MEM_BORROW_PERFORMANCE_0004
    RunLevel:
        Level 3
    EnvType:

    CaseName:
        004验证多次进行正常numa借用借用连续借用1024M性能
    PreCondition:
        P1.UBS-Engine进程正常拉起
        P2.MMI组件加载正常
        P3.UBS-Memory服务加载正常
    ...
    def test_tc_ubs_mem_borrow_performance_0004(self):
        ...
        self.logStep("S2.循环10次调用接口ubsmem_lease_malloc name 1024*1024*1024 0 64进行numa借用")
        mem_size = 1024 * 1024 * 1024
        addr_list = []
        for _ in range(10):
            addr_desc = self.host_nodes[0].apps[0].ubsmem_lease_malloc(
                self.default_region, mem_size, UbsMemInstance.DISTANCE_DIRECT_NODE, UBSM_FLAG_MALLOC_WITH_NUMA)

            self.logStep("E2.内存借用成功")
            self.assertEqual(addr_desc.rc, UBSM_SHMEM_OK)
            addr_list.append(addr_desc.addr)
