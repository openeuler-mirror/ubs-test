...
@pytest.mark.smoke
class TestTcUbsShmCreatePerformance0002(UbsMemCase):
    """
    CaseNumber:
        TC_UBS_SHM_CREATE_PERFORMANCE_0002
    RunLevel:
        Level 3
    EnvType:

    CaseName:
        002验证多次在非请求节点创建共享内存1024M性能
    PreCondition:
        P1.UBS-Engine进程正常拉起
        P2.MMI组件加载正常
        P3.UBS-Memory服务加载正常
    ...
    TestStep:
        S1.重启测试程序,获取进程pid
        S2.调用接口ubsmem_create_region name 0 0 2 host0 0 host1 1指定非请求节点导出内存
    ...
    def test_tc_ubs_shm_create_performance_0002(self):
        ...
        region_name = "TC_UBS_SHM_CREATE_PERFORMANCE_0002"
        shm_name_prefix = "TC_UBS_SHM_CREATE_PERFORMANCE_0002"
        size = 1024 * 1024 * 1024
        shm_name_list = []
