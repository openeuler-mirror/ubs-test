...
@pytest.mark.smoke
class TestTcMemFdCreateWithLenderSdk001(MEM_Pooling_BaseCase):
    """
    CaseNumber: 
        test_tc_mem_fd_create_with_lender_sdk_001.py
    RunLevel: 
        Level T
    EnvType: 
        
    CaseName: 
        验证sdk接口指定借出信息创建fd形态的远端内存成功
    PreCondition:
        P1.ubse进程已启动
        P2.节点集群状态为ok
    TestStep:
    ...
    def test_tc_fd_create_with_lender_test_name_001(self):

        self.logStep("S1.调用ubse_mem_fd_create_with_lender接口，参数合法")
        name = "mem_fd_with_lender_sdk_001"
