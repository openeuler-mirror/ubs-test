...
@pytest.mark.smoke
class TestTcMemFdGetSdk001(MEM_Pooling_BaseCase):
    """
    CaseNumber:
        test_tc_mem_fd_get_sdk_001
    RunLevel:
        Level T
    EnvType:
    ...
    PreCondition:
        ...
        P2.节点集群状态为ok
    TestStep:
        S1.调用ubse_mem_fd_create接口，参数合法
        S2.调用ubse_mem_fd_get查询S1中创建的内存，检查是否查询成功
        S3.调用ubse_mem_fd_delete接口删除指定fd远端内存,传入S1的name
        S4.调用ubse_mem_fd_get，传入S1的name，查看返回信息是否正确
    ExpectedResult:
        E1.内存创建成功
        E2.查询成功
        E3.内存删除成功
    ...
    def test_tc_mem_fd_get_sdk_001(self):

        name = "mem_fd_get_sdk_001"
        self.logStep("S1.调用ubse_mem_fd_create创建内存，检查是否创建成功")
        res = self.mem_fd_borrow(self.nodes[0], name=name)

        self.logStep("E1.内存创建成功")
        self.assertTrue(res, "内存创建失败")

        self.logStep("S2.调用ubse_mem_fd_get查询S1中创建的内存，检查是否查询成功")
        ...
        self.logStep("S4.调用ubse_mem_fd_get查询S1中创建的内存，检查是否查询成功")
        command = f"fd_get {name}"
        borrow_result = self.mem_borrow_common_result(node=self.nodes[0], command=command)
