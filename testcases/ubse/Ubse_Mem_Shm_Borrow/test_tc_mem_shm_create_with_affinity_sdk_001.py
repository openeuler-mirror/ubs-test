...
@pytest.mark.smoke
class TestTcMemShmCreateWithAffinitySdk001(MEM_Pooling_BaseCase):
    """
    CaseNumber:
        test_tc_mem_shm_create_with_affinity_sdk_001
    RunLevel:
        Level T
    EnvType:

    CaseName:
        验证sdk接口指定CPU平面创建共享内存成功
    PreCondition:
        P1.ubse进程已启动
        P2.节点集群状态为ok
    TestStep:
        S1.调用shm_create_with_affinity接口创建共享内存，参数合法
        S2.查看账本是否正确
        S3.归还内存，查看是否归还成功
    ExpectedResult:
        E1.内存借用成功
    ...
    def test_tc_mem_shm_create_with_affinity_sdk_001(self):

        self.logStep("S1.调用shm_create_with_affinity接口创建共享内存，参数合法")
        name = "mem_shm_create_with_affinity_sdk_001"
        ...
        cmd = f"shm_create {name} 128M --affinity_socket_id={socket_id}"
        mem_borrow = self.mem_borrow_common(self.nodes[0], cmd)
